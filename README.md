...
Vercel 使用 serverless function 运行 FastAPI。长时间搜索、引用扩展和大量请求建议使用 Docker 或 VPS。完整说明见 [Docker 与 Vercel 部署指南](docs/deployment.md)。
...
Web UI 中填写的 API Key、Base URL 和参数只用于当前浏览器会话，不写入本地配置文件。
